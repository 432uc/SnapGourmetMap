...
class _PhotoListScreenState extends State<PhotoListScreen> {
  ...
  PriceSearchMode _priceSearchMode = PriceSearchMode.firstItem;
  ...
  void _loadInitialData() {
    DBHelper.getData('categories').then((data) {
      if (mounted) {
        setState(() => _categories = data.map((item) => Category.fromMap(item)).toList());
      }
    });
    _performSearch(); 
  }
  ...
  void _resetSearch() {
    ...
    setState(() {
      ...
      _priceSearchMode = PriceSearchMode.firstItem;
    });
    _performSearch();
  }
          ...
          children: [
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Price Search Mode', style: Theme.of(context).textTheme.bodySmall),
                      SegmentedButton<PriceSearchMode>(
                        segments: const <ButtonSegment<PriceSearchMode>>[
                          ButtonSegment<PriceSearchMode>(value: PriceSearchMode.firstItem, label: Text('First Item')),
                          ButtonSegment<PriceSearchMode>(value: PriceSearchMode.total, label: Text('Total')),
                        ],
                        selected: <PriceSearchMode>{_priceSearchMode},
                        onSelectionChanged: (Set<PriceSearchMode> newSelection) {
                          setState(() {
                            _priceSearchMode = newSelection.first;
                          });
                        },
                      ),
                      Row(
            ...
            FutureBuilder<List<PhotoSpot>>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.isEmpty) return const Center(child: Padding(padding: EdgeInsets.all(16.0), child: Text('No matching photos found.')));
